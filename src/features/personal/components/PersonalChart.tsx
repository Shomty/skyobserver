...
const HIGHLIGHT = new Set(['Moon', 'Sun']);
...
export function PersonalChart({ positions, ascendantSign }: Props) {
  ...
  const highlightedPositions = useMemo(() => {
    return positions.map((p) => {
      if (!HIGHLIGHT.has(p.name)) return p;
      return { ...p, color: p.name === 'Moon' ? '#C0C0C0' : '#FFD700' };
    });
  }, [positions]);
