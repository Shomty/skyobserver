...
import type { ParashariSection } from '../lib/personalParashariEngine';
  ...
  return (
    <div className="space-y-3">
      {section.paragraphs.map((p) => (
        <p key={p.slice(0, 40)} className={cn('text-body leading-relaxed', muted)}>
          {p}
        </p>
      ))}
...
export function PersonalParashariPanel({ sections }: Props) {
  const { theme } = useTheme();
  const premiumUnlocked = usePersonalPremiumUnlocked();
