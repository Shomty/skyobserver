...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'sky' | 'chart' | 'stats' | 'archives' | 'profile' | 'report' | 'chat' | 'profiles'>('sky');
  ...
  useEffect(() => {
    if (activeTab === 'report' && routerLocation.pathname !== '/report') {
      ...
      navigate('/profiles');
    } else if (activeTab !== 'report' && activeTab !== 'chat' && activeTab !== 'profiles' && (routerLocation.pathname === '/report' || routerLocation.pathname === '/chat' || routerLocation.pathname === '/profiles')) {
      navigate('/');
    }
  ...
  useEffect(() => {
    ...
    } else if ((activeTab === 'report' || activeTab === 'chat' || activeTab === 'profiles') && routerLocation.pathname === '/') {
      setActiveTab('sky');
    }
...
- Use markdown for structure.`;
  ...
  const upcomingTransits = useMemo(() => {
    // Only re-calculate when the date changes or positions/birthPositions drastically change
    // Using startOfDay to keep it stable during the same calendar day
    return predictTransits(startOfDay(currentTime), positions, birthPositions || undefined);
  }, [format(currentTime, 'yyyy-MM-dd'), positions, birthPositions]);
